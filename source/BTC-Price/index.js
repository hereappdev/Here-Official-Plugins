...
function updateData() {
    ...
    http.get("https://production.api.coindesk.com/v1/currency/" + coinCode + "/ticker")
        .then((response) => {
            ...
            const lowPrice =
                unitSymbol +
                (usdPrice["low"] * rate > 100
                    ? (usdPrice["low"] * rate).toFixed(0)
                    : (usdPrice["low"] * rate).toFixed(2)) +
                "";
            const hightPrice =
                unitSymbol +
                (usdPrice["high"] * rate > 100
                    ? (usdPrice["high"] * rate).toFixed(0)
                    : (usdPrice["high"] * rate).toFixed(2)) +
                "";
